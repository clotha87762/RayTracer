...
bool IsInShadow(vec3 point , vec3 lightPos , Camera cam , vec3 normal);
vec3 SimpleMeshRayTrace(vec3 ori , vec3 dir , int depth ,Camera cam);
vec3 RayCastSynthesisColorImage(Camera  cam , vec2 xy);
void GenerateColorImage(Camera cam);
